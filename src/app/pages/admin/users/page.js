  ...
  return (
    ...
    <div className={`flex ${isLight ? 'bg-white' : 'bg-gray-800'}, ${isDark ? 'bg-gray-700 text-white' : 'bg-white'}`}>
      <nav className="w-1/5 h-screen bg-gray-800 text-white p-4">
        ...
        <ul className="mt-10">
          <li className="mb-4">
            <button onClick={userPage} className="w-full text-left px-4 py-2 bg-gray-700 rounded">Users</button>
          </li>
          <li className="mb-4">
      ...
      <div className="flex-1">
        ...
        <div className="grid justify-center mt-4">
          ...
          <div className="grid m-12 p-7 rounded-xl bg-slate-500">
          <div>
                  {user.map((article, index) => (
                    <div key={index} className={`bg-gray-100 p-4 mb-4 rounded shadow ${isDark ? 'bg-gray-300 text-black' : 'bg-white'}`}>
                      <h2 className="font-bold text-xl mb-2">{article.name}</h2>
                      <p className="text-gray-700">{article.email}</p>
                      <p className="text-gray-500 text-sm">{article.phone}</p>
                      <p className="text-gray-500 text-sm">{article.location}</p>
                      <p className="text-gray-500 text-sm">{article.password}</p>
                    </div>
                  ))}
                </div>
          </div>
